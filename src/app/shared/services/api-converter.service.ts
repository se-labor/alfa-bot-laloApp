...
export class ApiConverterService {
  ...
  getUserMessageFrom(text = '', payload = '', imageUrl = '', buttons: Array<BotButton> = []): Message {
  ...
  botResponseToMessage(botResponse: BotResponse): Message {
    ...
    const text = botResponse.message ? botResponse.message : '';
    const imageUrl = botResponse.imageUrl ? botResponse.imageUrl : '';
    const buttons = botResponse.buttons ? botResponse.buttons : [];
    return new Message(
